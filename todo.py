...
def complete(i):
    try:
        ...
        messagebox.showinfo("エラー","該当項目なし")
    return

#更新対応
def update():
    #入力欄とヘッダーと(ボタン)残して全部消す
    ...
    for i,task in enumerate(task_list):
        ...
        progress_box = ttk.Combobox(root,values = ["×","△","〇"],state ="readonly",width = 4)
        progress_box.set("×")
        progress_box.grid(row=i+1, column=4, sticky="ew")
...
priority_entry = ttk.Combobox(root,values = ["低","中","高"],state = "normal",width = 2,font=("明朝体", 22))
